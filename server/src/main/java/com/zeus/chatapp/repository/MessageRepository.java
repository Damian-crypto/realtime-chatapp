...
public interface MessageRepository extends JpaRepository<MessagePayload, Long> {
    ...
    @Query(
        value = "SELECT * FROM tbl_message WHERE receiver_id = ?1 ORDER BY tbl_message.timestamp DESC",
        nativeQuery = true
        )
    ...
    @Query(
        value = "SELECT * FROM tbl_message WHERE sender_id = ?1 ORDER BY tbl_message.timestamp DESC",
        nativeQuery = true
    )
    Optional<List<MessagePayload>> findBySenderId(Long senderId);

    @Query("SELECT msg FROM MessagePayload msg WHERE msg.sender.userId = :userId OR msg.receiver.userId = :userId ORDER BY msg.timestamp DESC")
